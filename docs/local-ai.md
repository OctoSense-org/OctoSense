...
Keep model weights in each user's MakeOS state directory, outside the Git checkout:

```text
~/.makeos/weights/Qwen3.5-9B-UD-Q4_K_XL.gguf
```

With `MAKEOS_HOME` set, use `<MAKEOS_HOME>/weights/` instead. Commit the setup instructions and app catalog; users install or link their own weights. The repository ignores `*.gguf` and `*.gguf.part` to avoid accidentally checking in large weights or links to personal paths.

## Make the assistant app available

These commands target the project's validated macOS setup and run from the MakeOS repository root. The default catalog launches `makepad-aichat` from `../guofoo-makepad`, with its model engine enabled by the app's default features. The model file alone does not install that app.

If that sibling checkout does not exist, create a fresh copy and select MakeOS's pinned dependency revision:

```sh
git clone https://github.com/guofoo/makepad.git ../guofoo-makepad
makepad_rev=$(python3 -c 'import json; print(json.load(open("upstream/makepad.json"))["dependency_revision"])')
git -C ../guofoo-makepad checkout --detach "$makepad_rev"
```
...
```sh
makeos_state_dir="${MAKEOS_HOME:-$HOME/.makeos}"
makeos_model="$makeos_state_dir/weights/Qwen3.5-9B-UD-Q4_K_XL.gguf"
mkdir -p "$makeos_state_dir/weights" &&
curl --fail --location --continue-at - \
  --output "$makeos_model.part" \
...
printf '%s  %s\n' \
  ...
  "$makeos_model.part" | shasum -a 256 -c - &&
mv -n "$makeos_model.part" "$makeos_model"
```
...
ln -s "$HOME/.makepad/weights/unsloth/Qwen3.5-9B-UD-Q4_K_XL.gguf" \
  "$makeos_state_dir/weights/Qwen3.5-9B-UD-Q4_K_XL.gguf"
```

Keep the source file in place. No symlink is needed if you select its absolute path when launching MakeOS:
...
Restart MakeOS with `cargo run` and press **F10**. The provider should show **Local · Qwen3.5 9B · local only**. Send a short prompt to check inference; the first answer loads the model. A separate model server or cloud API key is not required for this setup.
...
If the assistant is unavailable, check the catalog and sibling checkout. If the model is missing, check the effective `MAKEOS_HOME`, any `MAKEPAD_AI_CHAT_MODEL` override, and whether the model or symlink target exists. Restart after changing these paths. With multiple discovered GGUFs, the current runtime prefers Qwen files and then the larger file; use the explicit override to select one reliably.
