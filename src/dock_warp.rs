...
/// (never the window's overlay slot, which paints after every window), and
/// `request_window_gauss` inside resolves to a pyramid built from this very
/// texture (`CaptureGauss`). A presenter composites the texture and nothing
/// else, so a window's popups and glass stay in the window's stacking order.
